...
> Answers a specific confusion: NAVSIM is called "non-reactive" and "not closed-loop," yet it reports a PDM Score (PDMS) built from collisions, drivable-area compliance, and time-to-collision — metrics that sound closed-loop. How? See the companion notebook [navsim_pdms_mechanism.ipynb](navsim_pdms_mechanism.ipynb) for a from-scratch PDMS calculator and the correlation argument.
