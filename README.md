...
### CPU LLM fine-tuning notebook (`llm/domain_finetune_driving.ipynb`)

Real LoRA fine-tuning of a small causal LM (distilgpt2) — no GPU required.

```bash
uv sync --extra transformer --extra llm-cpu   # adds transformers + peft (CPU torch)
```
...
| `autonomous_driving/alpamayo2/alpamayo2.ipynb` | `--extra transformer` (CPU torch); ports NVIDIA's official Alpamayo modules, no alpamayo package or model weights needed |
| `autonomous_driving/alpamayo2/alpamayo2_gpu_smoke_kaggle.ipynb` | Kaggle GPU (T4) via Kaggle API — not run locally; contains outputs from an actual run |
...
| `agentic_engineering/harness_and_graph_engineering.ipynb` | core only (numpy/matplotlib; harness-variance, guide/sensor allocation, trilemma Pareto, coupling, code-graph retrieval, DAG scheduling simulations) |
...
│   └── qlora_gpu_smoke_kaggle.ipynb   # QLoRA pre-flight check on Kaggle T4: NF4 load, chat-template SFT, VRAM/throughput, adapter save/reload (real run outputs)
├── agentic_engineering/
...
│   ├── harness_and_graph_engineering.md  # Agent = Model + Harness: ETCLOVG 7-layer taxonomy, guides/sensors control view, cost-quality-speed trilemma + capability-control + coupling, code-graph retrieval (LocAgent/LARGER/Codebase-Memory), org vs work graphs
│   └── harness_and_graph_engineering.ipynb  # Six simulations: harness variance vs model upgrade, guide/sensor budget split, trilemma Pareto set, negative coupling interaction, graph-expansion recall/token economics, W/C speedup ceiling
└── autonomous_driving/
    ...
    │   ├── alpamayo2.md                        # Family (1 Nano/1.5/2 Super 34B), Chain-of-Causation, multi-camera tokenization, unicycle action space, flow-matching action expert, GRPO reward design
...
| Coding Agents, Harness Engineering & Graph Engineering | Three eras (prompt -> context -> harness -> graph engineering); `Agent = Model + Harness` and the evidence that harness swaps move pass@1 by 6-29 pp on fixed weights (62.4% -> 3.6% off-native worst case); ETCLOVG seven-layer taxonomy (Execution/Tooling/Context/Lifecycle/Observability/Verification/Governance) from the 170+ project survey and where open-source coverage is thin; Böckeler's inner/outer harness and guides (feedforward) vs sensors (feedback) x computational vs inferential, harnessability, keep-quality-left; closed-form guide/sensor allocation and the two anti-patterns; cost-quality-speed trilemma, capability-control tradeoff, harness coupling problem; code graphs as context (LocAgent 92.7% localization, LARGER +13.9 Acc@5 in-loop, Codebase-Memory 10x tokens / 2.1x tool calls at 83% vs 92% quality) and neighbourhood growth limits; org graph vs work graph, advisor-orchestrator / zone defense / council patterns, edge-failure modes, and the `W/C` speedup ceiling; ETCLOVG mapped onto a real harness with a sequencing rule (V before L, G before E) | [harness_and_graph_engineering.md](agentic_engineering/harness_and_graph_engineering.md) + [six simulations](agentic_engineering/harness_and_graph_engineering.ipynb) |
...
| LLM Fine-Tuning Methods — Survey & Practical Map | Four-stage pipeline (continued pretraining/SFT/alignment) x three PEFT families (additive: adapters/prompt/prefix/IA3; reparameterization: LoRA/DoRA/AdaLoRA/VeRA; selective: BitFit); QLoRA/NF4; SFT mechanics (prompt masking, chat templates, packing, LIMA); RLHF-PPO, DPO derivation, IPO/KTO/ORPO/SimPO, GRPO/RLVR; method decision guide; tooling (peft/trl/axolotl/unsloth/torchtune) | [finetuning_methods_survey.md](llm/finetuning_methods_survey.md) + [6-method head-to-head + from-scratch DPO](llm/finetuning_methods_survey.ipynb) / [Kaggle T4 QLoRA smoke test](llm/qlora_gpu_smoke_kaggle.ipynb) |
...
| NVIDIA Alpamayo 2 (Open Reasoning VLA) | Model family (1 Nano 10B / 1.5 Nano / 2 Super 34B, OpenMDW-1.1) and what actually changed between generations (RL weights and nav/VQA absent from the 1 Nano release; identical action space and normalization stats across all three; CFG dead in 1, implemented in 1.5, forbidden in 2); Chain-of-Causation (closed-set driving decisions, causal locality, 2-stage labeling); multi-camera tokenization (triplane/Flex, up to 20x compression); unicycle accel-curvature action space (64 waypoints @6.4s, low-speed curvature clamp); flow-matching action expert reusing the VLM KV cache (8.75 ms vs 222 ms autoregressive); 3-stage recipe with GRPO and a reasoning-action consistency reward | [alpamayo2.md](autonomous_driving/alpamayo2/alpamayo2.md) + [CPU port of the official modules](autonomous_driving/alpamayo2/alpamayo2.ipynb) / [Kaggle T4 GPU smoke test](autonomous_driving/alpamayo2/alpamayo2_gpu_smoke_kaggle.ipynb) |
