...
**QLoRA** (Dettmers 2023) = base weights in 4-bit **NF4** (a quantile-optimal 4-bit float for normally-distributed weights) + double quantization of the scale constants + paged optimizer states + LoRA adapters in bf16. Gradients flow *through* the dequantized weights into the adapters; the base never updates, so quantization error never accumulates. Result: 65B fine-tuning on a single 48 GB card, 7B on a free Colab T4, with quality matching 16-bit LoRA on instruction benchmarks. Full mechanics: [lora_qlora.md §4–5](lora_qlora.md); GPU recipe: [lora_qlora_finetune.ipynb](lora_qlora_finetune.ipynb).
